...
abstract class NumberProvider<T extends Number> implements Provider<T> {
   ...
   */
  protected static void validate(double number, List<? extends Annotation> modifiers)
      ...
      if (modifier instanceof Range) {
        ...
        if (number < range.min()) {
          throw new ArgumentParseException(String
              .format("A valid value is greater than or equal to %s (you entered %s)", range.min(),
                  number));
        } else if (number > range.max()) {
          throw new ArgumentParseException(String
              .format("A valid value is less than or equal to %s (you entered %s)", range.max(),
                  number));
        }
      }
    }
   ...
   * @throws ArgumentParseException on a validation error
   */
  protected static void validate(int number, List<? extends Annotation> modifiers)
      throws ArgumentParseException {
    for (Annotation modifier : modifiers) {
      if (modifier instanceof Range) {
        Range range = (Range) modifier;
