...
public final class ArgumentParser {
  ...
  private ArgumentParser(List<ParameterEntry> parameters, List<Parameter> userParams,
   ...
   * @throws ProvisionException If there is a problem with the binding itself
   */
  public Object[] parseArguments(CommandArgs args, boolean ignoreUnusedFlags,
      Set<Character> unusedFlags) throws ArgumentException, ProvisionException {
    ...
    for (int i = 0; i < parameters.size(); i++) {
      ...
      try {
        parsedObjects[i] = entry.getBinding().getProvider()
            .get(argsForParameter, entry.getModifiers());
  ...
  private void checkUnconsumed(CommandArgs arguments, boolean ignoreUnusedFlags,
      Set<Character> unusedFlags) throws UnusedArgumentException {
  ...
  public static class Builder {
    ...
    public void addParameter(Type type, List<? extends Annotation> annotations)
      ...
      if (binding == null) {
        throw new IllegalParameterException(
            "Can't finding a binding for the parameter type '" + type + "'");
      }
    ...
    private static String getFriendlyName(Type type, Annotation classifier, int index) {
      if (classifier != null) {
        ...
        return type instanceof Class<?> ? ((Class<?>) type).getSimpleName().toLowerCase()
            : "unknown" + index;
      }
    }
  }
  ...
  private static class ParameterEntry {
    ...
    ParameterEntry(Parameter parameter, Key<?> key, Binding<?> binding,
        List<Annotation> modifiers) {
