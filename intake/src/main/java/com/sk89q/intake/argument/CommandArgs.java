...
public interface CommandArgs {
   ...
   * @throws MissingArgumentException Thrown if there are no remaining arguments
   * @throws ArgumentParseException Thrown if the next argument could not be parsed to an
   *     integer
   */
   ...
   * next call to {@link #next()} and it starts at 0. If the current position is equal to {@link
   * #size()}, then there are no more arguments available.</p>
   *
   ...
   * the flag. For boolean flags, the value should be the string "true". Flags are commonly defined
   * by the player by using syntax similar to {@code -X value}.</p>
   *
   ...
   * parsing has begun and they can be used by commands or {@link Provider} to get session-related
   * values.</p>
   *
